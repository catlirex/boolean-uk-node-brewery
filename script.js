...
app.get("/breweries", (req, res) => {
  ...
  if (!queryContent) res.json({ breweries });
  else if (!queryContent.brewery_type) res.json({ Error: "Wrong Query Input" });
  else {
...
app.get("/breweries/:id", (req, res) => {
  let filteredBreweries = breweries.find(
    (target) => target.id === Number(req.params.id)
  );
  if (!filteredBreweries) res.json({ Error: `ID:${req.params.id} Not found` });
  res.json({ filteredBreweries });
});
